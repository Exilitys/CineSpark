...
export const useStory = (projectId: string | null) => {
    ...
    characters: [
      {
        name: "Marcus",
        description:
          "A weathered 45-year-old former marine biologist turned lighthouse keeper",
        motivation:
          "To find redemption and purpose after losing his research team in a diving accident",
        arc: "From isolated and guilt-ridden to connected and purposeful",
      },
      {
        name: "Naia",
        description:
          "An intelligent sea creature from an ancient underwater civilization",
        motivation:
          "To save her dying people and forge understanding between species",
      ...
      },
      {
        ...
        motivation:
          "To bring Marcus back to the scientific community and continue their work",
    ...
    scenes: [
      {
        ...
        description:
          "Marcus performs his daily maintenance routine with mechanical precision",
        characters: ["Marcus"],
        key_actions: [
          "Checking lighthouse equipment",
          "Making coffee",
          "Looking out at empty ocean",
        ],
      },
      {
        title: "The Discovery",
        ...
        description:
          "Marcus finds Naia unconscious on the beach, making the choice to help",
  ...
  const fetchStory = async () => {
    ...
    try {
      ...
      if (
        error?.code === "PGRST116" ||
        (error?.body && JSON.parse(error.body)?.code === "PGRST116")
      ) {
        try {
  ...
  return {
    ...
    updateScene,
    refetch: fetchStory,
    addCharacter, // ✅ expose this
    addScene, // ✅ expose this
  };
};
